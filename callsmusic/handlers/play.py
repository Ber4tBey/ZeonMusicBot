...
async def play(_, message: Message):
    ...
    response = await message.reply_text('<b>🔄 İşleniyor...</b>', False)
    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f'Video süresi {DURATION_LIMIT} dakikadan uzun '
                'Bu süredeki ses oynatılamaz !!'
                f'{round(audio.duration / 60)} dakika'
            )
        ...
        if urls:
            ...
            await response.edit_text(
                '<b>❌ Oynatmam için birşeyler versen canım bi zahmet </b>',
            )
            return
    ...
    if chat_id in callsmusic.active_chats:
        ...
        await response.edit_text(
            f'<b>#️⃣ Sıraya alındı. Sıra no: {position}</b>...',
        )
    else:
        await callsmusic.set_stream(chat_id, file)
        await response.edit_text('<b>▶️ Oynatılıyor...</b>')
